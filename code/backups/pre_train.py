...
def get_test_resizer(dataset_name):    
    test_resizer = A.Compose([Resize(*TEST_INPUT_SIZE[dataset_name])]) 
    return test_resizer
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'test':
            ...
            return totensor(cropped_img), totensor(gt), self.data[idx].bbox, self.data[idx].ID
...
class StepRunner:
    ...
    def step(self, features, labels):
        ...
        #beta = labels.sum() / labels.shape[1] * labels.shape[2]
        with autocast():
            preds = self.net(features)
            loss = self.loss_fn(preds,labels.squeeze(1).long())
...
if __name__ == '__main__':
    ...
    TRAIN_DATASETS = [ 'HRF']    
    #TRAIN_INPUT_SIZE = {'DRIVE': [512, 512], 'CHASEDB':[1024, 1024], 'HRF':[1024, 1024], 'STARE':[1024, 1024]}
    TRAIN_INPUT_SIZE = {'DRIVE': [584, 565], 'CHASEDB':[960, 999], 'HRF':[1024, 1552], 'STARE':[605, 700]}
    ...
    for TRAIN_DATASET in TRAIN_DATASETS:
        ...
        transforms = A.Compose([RandomRotate90(p = 0.5), Flip(p = 0.5),
                                Affine(scale = (0.95, 1.2), translate_percent = 0.05, rotate = (-45, 45)),
                                ColorJitter(brightness, contrast, saturation, hue, always_apply = True)])
...
# In[]
    ...
    #TEST_DATASETS = ['DRIVE', 'CHASEDB', 'HRF']
